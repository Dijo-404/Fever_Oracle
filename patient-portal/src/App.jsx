...
import { Info, Map, MessageSquare, AlertCircle, Activity } from 'lucide-react'

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000'

function App() {
  const [selectedFeverType, setSelectedFeverType] = useState('all')
  const [localAlerts, setLocalAlerts] = useState([])

  const handleChatbotComplete = (analysis) => {
    fetchAlerts()
  }
  ...
  const fetchAlerts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.alerts) {
          setLocalAlerts(data.alerts.slice(0, 3))
        }
      }
    } catch (error) {
      ...
      <header className="border-b border-border bg-card">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">FEVER ORACLE</h1>
